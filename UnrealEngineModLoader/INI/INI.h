...
template <typename T> T convert_to(const fini_string_t &str)
{
...
template <> fini_string_t convert_to<fini_string_t>(const fini_string_t &str)
{
    return str;
}

template <> const char *convert_to<const char *>(const fini_string_t &str)
{
    return str.c_str();
}
...
class INI
{
  public:
    ...
    {
        SAVE_PRUNE = 1 << 0,
        SAVE_PADDING_SECTIONS = 1 << 1,
        SAVE_SPACE_SECTIONS = 1 << 2,
        SAVE_SPACE_KEYS = 1 << 3,
        SAVE_TAB_KEYS = 1 << 4,
        SAVE_SEMICOLON_KEYS = 1 << 5
    };
    enum parse_e
    {
        PARSE_COMMENTS_SLASH = 1 << 0,
        PARSE_COMMENTS_HASH = 1 << 1,
        PARSE_COMMENTS_ALL = 1 << 2
    };
    ...
    template <class T> T getAs(fini_string_t section, fini_string_t key, T def = T());
    template <class T> T getAs(fini_string_t key, T def = T());
...
template <class T> T INI::getAs(fini_string_t section, fini_string_t key, T def)
{
    return getAs<T>(key, def);
}

template <class T> T INI::getAs(fini_string_t key, T def)
{
    auto it = current->find(key);
    if (it == current->end())
        return def;
