...
class TNameEntryArray
{
  ...
  private:
    ...
    {
        return Read<int>((byte *)this + 0x8 + 0x4);
    };
    FNameEntry **Chunks[];
};
...
class FName
{
  public:
    ...
    inline FName(int i) : ComparisonIndex(i), Number(0){};
    ...
    LOADER_API static DWORD *GNames;
...
template <typename Key, typename Value> class TMap
{
    char UnknownData[0x50];
};

template <class T, class TBASE> class TAutoPointer : public TBASE
{
  public:
...
struct FActorSpawnParameters
{
    FActorSpawnParameters()
        : Name(""), Template(NULL), Owner(NULL), Instigator(NULL), OverrideLevel(NULL), bNoCollisionFail(false),
          bRemoteOwned(false), bNoFail(false), bDeferConstruction(false), bAllowDuringConstructionScript(false),
